...
$stmt = $pdo->prepare("SELECT * FROM signalements WHERE user_id = ? ORDER BY date_signalement DESC");
...
<body>
  ...
  <div class="header">
    <div>
      <a href="tableau_de_bord.php"><i class="fas fa-arrow-left"></i> Retour</a>
      <a href="tableau_de_bord.php"><i class="fas fa-home"></i></a>
    </div>
    <span style="font-weight: bold;">Mes Signalements</span>
  ...
  <div class="container">
    <div class="title">
      ...
      <p>Cliquez sur "Voir" pour afficher tous les détails</p>
    </div>
    ...
    <!-- FILTRES ET RECHERCHE - AJOUTÉ -->
    <div class="filters">
      <div class="search-container">
        <input type="text" id="search" placeholder="Rechercher dans mes signalements..." onkeyup="filterRows()">
      </div>
  ...
  <!-- SCRIPT SIMPLE AVEC FONCTIONS DE FILTRAGE -->
  <script>
    // Données depuis PHP
    const signalements = <?php echo json_encode($signalements); ?>;
    
    // FONCTIONS DE FILTRAGE - NOUVELLES
    ...
    function filterBy(type, value) {
      ...
      rows.forEach(row => {
        ...
        if (value !== '') {
          if (type === 'cat') {
            // 3ème colonne (index 2) pour catégorie
            const cat = row.cells[2].innerText;
            show = cat.includes(value);
          } else if (type === 'statut') {
            // 5ème colonne (index 4) pour statut
